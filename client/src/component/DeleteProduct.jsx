...
import { BiLoaderAlt } from "react-icons/bi"; // <-- Ye import missing tha!
import toast from 'react-hot-toast';
import Axios from '../utils/Axios';
import SummaryApi from '../common/SummaryApi';

const DeleteProduct = ({ data, close, fetchProductData }) => {
    ...
    const handleConfirm = async () => {
        setLoading(true);
        try {
            // SummaryApi me deleteProduct ka path hona zaroori hai
            ...
            if (response.data.success) {
                ...
                fetchProductData(); // Table refresh
            ...
            >
                ...
                <div className="p-7 text-center">
                    ...
                    <p className="text-sm text-gray-500 mb-8 leading-relaxed px-2">
                        Are you sure you want to delete <span className="font-semibold text-gray-700">"{data.name}"</span>? This action is permanent and cannot be undone.
                    </p>
                    ...
                    <div className="flex justify-center gap-3">
                        ...
                        <button
                            ...
                            disabled={loading}
                            className="flex-1 px-4 py-3 bg-red-500 text-white font-semibold rounded-xl hover:bg-red-600 shadow-[0_8px_15px_-3px_rgba(239,68,68,0.3)] hover:shadow-[0_12px_20px_-3px_rgba(239,68,68,0.4)] transition-all duration-200 active:scale-95 flex justify-center items-center"
                        >
                            {loading ? <BiLoaderAlt className="animate-spin text-xl" /> : 'Yes, Delete'}
                        </button>
                    </div>
                </div>
            </div>
