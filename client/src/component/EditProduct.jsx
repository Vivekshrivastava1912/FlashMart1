...
const EditProduct = ({ data: initialData, close, fetchProductData }) => {
  ...
  // State Initialization: Category/SubCategory ko array format me convert kiya taki map error na de
  ...
  const handleUploadImage = async (e) => {
    ...
    try {
      ...
      setData((preve) => ({ ...preve, image: [...preve.image, ImageResponse.data.url] }));
    } catch (error) {
  ...
  const handleCategoryChange = (e) => {
    ...
    if(categoryDetails && !data.category.some(el => el._id === value)) {
        setData((preve) => ({ ...preve, category: [categoryDetails] })); 
    }
  };
  ...
  const handleSubCategoryChange = (e) => {
    ...
    if(subCategoryDetails && !data.subCategory.some(el => el._id === value)) {
        setData((preve) => ({ ...preve, subCategory: [subCategoryDetails] }));
    }
  };
  ...
  const handleSubmit = async(e) => {
    ...
    try {
        // ERROR FIXING: Backend ko sirf String ID bhej rahe hain
        const payload = {
            ...data,
            category: data.category.map(c => typeof c === 'string' ? c : c._id)[0],
            subCategory: data.subCategory.map(s => typeof s === 'string' ? s : s._id)[0],
  ...
  return (
    <section className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm transition-all duration-300">
        ...
        <div className="p-4 md:p-5 flex-1 overflow-y-auto bg-gray-50">
          <form className="flex flex-col gap-4 md:gap-3 bg-white p-4 md:p-5 rounded-lg border border-gray-200 shadow-sm" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <input type="text" name="name" value={data.name} onChange={handleChange} required className="w-full border border-gray-300 rounded-md p-2 md:p-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-purple-500" />
            </div>
                  ...
                  <div className='flex flex-wrap gap-2'>
                     {data.category.map((cat, i) => (
                          <div key={i} className='flex items-center gap-1 bg-purple-50 text-purple-700 px-2 py-1 rounded border border-purple-200 text-xs font-medium'>
                              {cat.name}
                              <MdClose className='cursor-pointer hover:text-red-500' onClick={() => setData(prev => ({...prev, category: []}))} />
                          </div>
                      ))}
                  </div>
                </div>
                  ...
                  <div className='flex flex-wrap gap-2'>
                     {data.subCategory.map((sub, i) => (
                          <div key={i} className='flex items-center gap-1 bg-blue-50 text-blue-700 px-2 py-1 rounded border border-blue-200 text-xs font-medium'>
                              {sub.name}
                              <MdClose className='cursor-pointer hover:text-red-500' onClick={() => setData(prev => ({...prev, subCategory: []}))} />
                          </div>
                      ))}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-3">
              <div>
                ...
                <select name="unit" value={data.unit} onChange={handleChange} required className="w-full border border-gray-300 rounded-md p-2 md:p-1.5 text-sm outline-none focus:ring-1 focus:ring-purple-500">
              ...
              <div>
                ...
                <input type="number" name="stock" value={data.stock} onChange={handleChange} required className="w-full border border-gray-300 rounded-md p-2 md:p-1.5 text-sm outline-none focus:ring-1 focus:ring-purple-500" />
              </div>
              <div>
                <label className="block text-sm md:text-xs font-semibold text-gray-700 mb-1">Price (₹)</label>
                <input type="number" name="price" value={data.price} onChange={handleChange} required className="w-full border border-gray-300 rounded-md p-2 md:p-1.5 text-sm outline-none focus:ring-1 focus:ring-purple-500" />
              </div>
              <div>
                <label className="block text-sm md:text-xs font-semibold text-gray-700 mb-1">Discount (%)</label>
                <input type="number" name="discount" value={data.discount} onChange={handleChange} required className="w-full border border-gray-300 rounded-md p-2 md:p-1.5 text-sm outline-none focus:ring-1 focus:ring-purple-500" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-3">
              <div>
                ...
                <div className='flex gap-2 flex-wrap'>
                  <label className="w-16 h-16 flex items-center justify-center border-2 border-gray-300 border-dashed rounded-md cursor-pointer bg-gray-50 hover:bg-gray-100">
                    {loading ? <BiLoaderAlt className='animate-spin text-purple-600' /> : <span className="text-xl text-purple-600">+</span>}
                    <input type="file" className="hidden" onChange={handleUploadImage} disabled={loading} />
                  </label>
                  {data.image.map((img, i) => (
                    <div key={i} className='relative w-16 h-16 border rounded bg-white overflow-hidden shadow-sm'>
                      <img src={img} className='w-full h-full object-contain' alt="product" />
                      <button type="button" onClick={() => handleRemoveImage(i)} className='absolute top-0 right-0 bg-red-500 text-white p-0.5 rounded-bl'><MdClose size={12}/></button>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                ...
                <textarea name="discription" value={data.discription} onChange={handleChange} required className="w-full border border-gray-300 rounded-md p-2 text-sm focus:outline-none focus:ring-1 focus:ring-purple-500 h-24 resize-none"></textarea>
              </div>
            </div>
