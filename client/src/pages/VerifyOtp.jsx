...
const VerifyOtp = () => {
  ...
  const location = useLocation() // Email lene ke liye state se
  const [otp, setOtp] = useState(["", "", "", "", "", ""])
  const inputRefs = useRef([])

  // Safety check: Agar email nahi hai toh wapas bhej do
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await Axios({
        ...
        data: {
          otp: fullOtp,
          email: location?.state?.email // YAHAN EMAIL BHEJNA ZAROORI HAI
        }
      })
      ...
      if (response.data.success) {
        ...
        navigate("/") // Successful verify hone par home page
      }
    } catch (error) {
      ...
      <div className='bg-white w-full max-w-sm sm:max-w-md p-6 sm:p-8 rounded-2xl shadow-2xl border border-purple-100 relative'>
        ...
        <form onSubmit={handleSubmit} className='space-y-8'>
          <div className='flex justify-center gap-2 sm:gap-3'>
            {otp.map((data, index) => (
              <input 
                key={index} type="text" maxLength="1" inputMode="numeric"
